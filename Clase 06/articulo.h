...
void listar_articulos_ordenados();
...
void ordenar_articulos(Articulo*v, int cant){
  int max,i,j, aux;
  Articulo a;
  for (i=0;i<cant-1;i++){
      aux=i;
    for (j=i+1;j<cant;j++){
      if (v[j].stock<v[aux].stock) {
        aux=j;
      }
    }
    a=v[i];
...
void listar_articulos_ordenados(){
  system("cls");
  int cant = cantidad_articulos();
  Articulo *v;
  v = (Articulo *) malloc (cant * sizeof(Articulo));
  FILE * p = fopen("articulos.dat", "rb");
  if (p == NULL || v == NULL){
    cout << "No se puede continuar.";
    return;
  }
  fread(v, sizeof(Articulo), cant, p);
  fclose(p);

  ordenar_articulos(v, cant);
  cout << "Listado con memoria dinámica" << endl;
  cout << endl;
  for(int i=0; i<cant; i++){
    mostrar_articulo(v[i]);
    cout << endl<< "------------------" << endl;
  }
  free(v);
}
